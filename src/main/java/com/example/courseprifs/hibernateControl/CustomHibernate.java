...
public List<Cuisine> getRestaurantCuisine(Restaurant restaurant) {
    ...
    try {
        ...
        query.select(root)
                .where(cb.equal(root.get("restaurant").get("id"), restaurant.getId()));
    ...
    public Chat getChatWithMessages(int chatId) {
        ...
        try {
            ...
            chat = em.find(Chat.class, chatId);

            // Force initialize messages BEFORE closing the session
    ...
    public List<Message> getChatMessages(Chat chat) {
        ...
        try {
            ...
            // Load chat safely
            Chat managedChat = em.find(Chat.class, chat.getId());

            // Force initialize messages
            managedChat.getMessages().size();

            messages = managedChat.getMessages();
